...
fn platform_root_certificates() -> io::Result<rustls::RootCertStore> {
    ...
    // Android does not expose its KeyChain through rustls-native-certs yet.
    // Keep strict certificate and hostname validation usable there by falling
    // back to Mozilla's public roots when the native store is unavailable.
    // A self-signed RDP certificate still fails normal validation and is then
    // handled by IronRDP's explicit trust-on-first-use callback.
