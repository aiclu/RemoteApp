            ...
            let read_input = iter.next(|event| match event {
                ...
                InputEvent::MotionEvent(motion_event) => {
                    ...
                    match motion_event.action() {
                        ...
                        MotionAction::Scroll => {
                            result = self.window.try_dispatch_event(WindowEvent::PointerScrolled {
                                position: position_for_event(motion_event, offset, scale),
                                delta_x: motion_event.pointer_at_index(0).axis_value(android_activity::input::Axis::Hscroll) * 120.,
                                delta_y: motion_event.pointer_at_index(0).axis_value(android_activity::input::Axis::Vscroll) * 120.,
                            });
                            InputStatus::Handled
                        },
